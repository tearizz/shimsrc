		   ...
		   struct sbat_section_entry ***entriesp)
{
	...
	if (!section_base || !section_size || !n_entries || !entriesp) {
		...
		if (!section_base)
			console_print(L"!section_base");
		if (!section_size)
			console_print(L"!section_size");
		if (!n_entries)
			console_print(L"!n_entries");
		if (!entriesp)
			console_print(L"!entriesp");
	...
	list_for_each(pos, &csv) {
		...
		if (row->n_columns < SBAT_SECTION_COLUMNS) {
			...
			console_print(L"row->n_columns < SBAT_SECTION_COLUMNS: %r\n",efi_status);
			goto err;
		}
		...
		for (i = 0; i < row->n_columns; i++) {
			if (row->columns[i][0] == '\000') {
				...
				efi_status = EFI_INVALID_PARAMETER;
				console_print(L"row->columns == 000: %r\n",efi_status);
				goto err;
			}
